...
def scale(img, w, h):
    """Scale image to specified width/height without smoothing."""
...
BOSS_END   = (int(WIDTH*0.52), int(HEIGHT*0.60))
BOSS_FAR   = (28, 42)
BOSS_NEAR  = (120, 180)
...
play = {
    "score": 0.0,
    "phone": False,       # True = show laptop w/ phone overlay
    "gameover": False,
    "caught": False,
    "boss_state": WAIT,
    "boss_timer": 0.0,
...
while running:
    ...
    if scene == SCENE_PLAY and not play["gameover"]:
        ...
        # Boss logic
        play["boss_timer"] += dt
        if play["boss_state"] == WAIT and play["boss_timer"] >= play["next_check_in"]:
            play["boss_timer"] = 0.0
    ...
    if scene == SCENE_LEVEL_SELECT:
        ...
        draw_text(screen, font, f"Level {selected_level}  |  Score: {int(play['score'])}  |  x{params['mult']:.2f}", 16, 14, (255, 255, 255))
